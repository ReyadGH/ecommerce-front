    ...
    <>
      <div className=" text-center p-3">
        ...
        <select
          ...
          id=""
          onChange={(e) => {
            changeHandler({ ...pageble, target: e.target.value });
            console.log(pageble.target);
          }}
        >
      ...
      {pageble.result.length != 0 ? (
        <SimpleTable items={pageble.result} actions={[]} />
      ) : (
        <p className="text-center p-3">
          No result for, {pageble.target} = "{pageble.value}"
        </p>
      )}
